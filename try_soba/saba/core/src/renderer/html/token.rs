...
pub struct HtmlTokenizer {
    state: State,    // state of state machine
    pos: usize,      // position of text currently processed
    reconsume: bool, // only update state and reuse character already processed.
    latest_token: Option<HtmlToken>,
    input: Vec<char>, // string of HTML itself
    buf: String,
}
...
// Separate HTML into tokens.
// There are 6 tokens, DOCTYPE, beginning tag, end tag, comment, string and EOF.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum HtmlToken {
    StartTag {
